  ...
  return (
    <>      
      <form name="FrmCadastroAluno" method="post" onSubmit={alterarId < 0 ? handleSubmitInsert: handleSubmitUpdate}>
          ...
          <label>Curso: 
          <input type="text" size="50" id="curso" name="curso" value={curso} onChange={(event) => setCurso(event.target.value)} /></label><br/>
          <label>CPF: 
...
function FrmListarAluno(){
  ...
  return (
    <div>
        ...
        <div>
          <table border='1'> 
            <td>Id</td> <td>Nome</td> <td>Curso</td> <td>CPF</td> <td>Editar</td> <td>Excluir</td>          
            {alunos.map(aluno => (
              <tr>
                <td> {aluno.alunoId} </td>
                <td> {aluno.nome}</td>
